...
- Vehicle under test: VIN `5YJSA00000F000000`, 2022 Model S "Roadrunner", `command_signing: required`.
...
- Create: `com.example.tesla-proxy.plist`

**Interfaces:**
- Consumes: `keys/private-key.pem` — the **same** pair registered at `example.com`. Using a different key makes every command fail at the car, and Tesla does not support keypair rotation.
...
1. Open `https://tesla.com/_ak/example.com` on the phone with the Tesla app (4.27.3+) installed.
2. Approve adding the key to "Roadrunner".

This works because the public half is already served at
`https://example.com/.well-known/appspecific/com.tesla.3p.public-key.pem`, which the app fetches at pairing time.
...
Expected: `key_paired_vins` contains `5YJSA00000F000000`. If it is empty, the pairing did not complete — repeat Step 3 with the car awake.
...
curl -sS --cacert keys/tls-cert.pem \
  ...
  --data '{}' \
  "https://localhost:4443/api/1/vehicles/5YJSA00000F000000/command/flash_lights"
```
...
<dict>
  <key>Label</key>
  <string>com.example.tesla-proxy</string>
  <key>ProgramArguments</key>
  <array>
...
cp /Users/d/Code/tesla_automation/com.example.tesla-proxy.plist ~/Library/LaunchAgents/
launchctl unload ~/Library/LaunchAgents/com.example.tesla-proxy.plist 2>/dev/null
launchctl load ~/Library/LaunchAgents/com.example.tesla-proxy.plist
sleep 3
...
```bash
git add com.example.tesla-proxy.plist .gitignore
        ...
        if "has not been paired" in error or "UNKNOWN_KEY_ID" in error:
            return {"ok": False, "reason": error,
                    "message": "The car has not been paired with this app's key. "
                               "Open tesla.com/_ak/example.com on your phone."}
...
```bash
launchctl unload ~/Library/LaunchAgents/com.example.tesla-proxy.plist
lsof -ti:8000 | xargs kill -9 2>/dev/null
cd /Users/d/Code/tesla_automation && .venv/bin/python app.py > /tmp/app.log 2>&1 &
sleep 2
curl -s localhost:8000/api/car/health
    ...
    try:
        ...
        try:
            ...
            print(f"  {BAD} signing proxy unreachable — launchctl load "
                  f"~/Library/LaunchAgents/com.example.tesla-proxy.plist")
            ok = False
        ...
        try:
            ...
            if snap:
                ...
                print(f"  {BAD} no samples recorded — launchctl load "
                      f"~/Library/LaunchAgents/com.example.tesla-collector.plist")
                ok = False
        finally:
            store.close()
    finally:
