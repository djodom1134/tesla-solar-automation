...
| `com.example.tesla-collector.plist` (create) | launchd agent | 2 |
...
**Files:**
- Create: `collector.py`, `tests/test_collector.py`, `com.example.tesla-collector.plist`
- Modify: `.gitignore`
...
Create `com.example.tesla-collector.plist`:
...
<dict>
  <key>Label</key>
  <string>com.example.tesla-collector</string>
...
```bash
cp /Users/d/Code/tesla_automation/com.example.tesla-collector.plist ~/Library/LaunchAgents/
launchctl unload ~/Library/LaunchAgents/com.example.tesla-collector.plist 2>/dev/null
launchctl load ~/Library/LaunchAgents/com.example.tesla-collector.plist
sleep 5
...
```bash
git add collector.py tests/test_collector.py com.example.tesla-collector.plist .gitignore
...
## Task 16: "Free miles" — distance banked from the sun

The owner's framing: *how far could we go for free, on sun stored in the
battery?* That is a different question from range, and a better one.

**It is three numbers, and the honesty is the feature.** Every one of them is
`None` until the data supports it — the card says "collecting" rather than
showing a confident wrong figure. Today the honest answer is **0 free miles**,
because the controller has never charged from surplus yet.

**Files:**
- Create: `green.py`, `tests/test_green.py`
- Modify: `solar_routes.py` (one field on `/solar/status`), `static/car.js`,
  `demo.py`

**Interfaces:**
- `green.solar_kwh(ticks: list[dict]) -> float`
- `green.pack_kwh(sessions: list[dict]) -> tuple[float | None, int]`
- `green.miles_per_kwh(segments: list[dict], pack_kwh: float | None) -> tuple[float | None, float]`
- `green.free_miles(solar_kwh, mi_per_kwh) -> float | None`

- [ ] **Step 1: Write the failing tests**

Create `tests/test_green.py`. The properties that matter:
...
Expected: `ModuleNotFoundError: No module named 'green'`

- [ ] **Step 3: Write `green.py`**

Pure functions only — no database, no network, no clock. `collector.py` and
`solar_routes.py` supply the rows. Thresholds to encode, each with the reason
in the docstring:

- `MIN_SESSIONS = 2`, `MIN_SESSION_SOC = 10` — below this the integer-percent
  quantisation dominates.
- `MIN_MILES = 50` — a shorter sample is weather and terrain, not consumption.
- Every function returns `None` rather than a guess, and returns its own sample
  size alongside so the caller can say how sure it is.

- [ ] **Step 4: Surface it honestly**

Add to `/api/car/solar/status`: `free_miles`, `solar_kwh_today`,
`mi_per_kwh`, `miles_sampled`, `pack_kwh`, `pack_sessions`. Keep demo/real key
parity — there is a test asserting the key sets match exactly.

On the card, when `free_miles` is `None`, say what is missing rather than
blanking: *"Free miles: collecting — need 50 miles of driving (have 43.6) and
2 charge sessions (have 1)."* A number that is absent for a stated reason is
more useful than one that is absent silently, and far more useful than one that
is wrong.
...
**Verified live against the owner's device** at `http://10.0.0.78/`:
