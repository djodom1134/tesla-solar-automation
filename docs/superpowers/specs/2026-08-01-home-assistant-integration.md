...
| Garage cover (task 9) | Mac mini → 192.168.87.78 | **macOS Local Network gate: BLOCKED.** Requires the one GUI grant. Deferred behind it |
...
- **R1** `http://192.168.87.84:8000/api/ha/state` — `scan_interval: 60`
- **R2** `http://192.168.87.84:8000/api/ha/meters` — `scan_interval: 300`
...
3. Verify from a laptop: `curl http://192.168.87.84:8000/api/car/home` → **401**. If it returns 200, stop — the middleware is not live and the garage/coordinates hole is still open.
...
12. Re-run `curl -H 'X-Api-Key: …' http://192.168.87.84:8000/api/car/garage` and confirm `reachable: true` before pasting the cover YAML.
