...
| `ha.the ownerodom.com` is publicly reachable with a valid cert | `curl -w '%{http_code} %{ssl_verify_result}'` | `200`, `ssl_verify=0`, 59 ms. The hard prerequisite for a self-hosted Google Action is already met |
| HA is up on the LAN | `curl http://10.0.0.6:8123/` | `403` — HA's trusted-proxy rule, not a failure |
...
ratgdo at `10.0.0.78`. Everything here is designed for one. A second
...
`https://ha.the ownerodom.com/api/google_assistant`, OAuth account linking, and a
...
4. From a laptop: `curl http://10.0.0.84:8000/api/car/home` → **401**. If it
   returns 200, stop — the middleware is not live.

**To connect Claude:**

5. Claude Code: `claude mcp add --transport http tesla http://10.0.0.84:8000/mcp --header "X-Api-Key: <API_TOKEN_MCP>"`.
...
8. Re-run `curl -H 'X-Api-Key: …' http://10.0.0.84:8000/api/car/garage` and
   confirm `reachable: true` before wiring the cover.

**For Google (task 10):**

9. Create a Google Cloud project and a smart-home Action; set fulfilment to
   `https://ha.the ownerodom.com/api/google_assistant`; configure account linking;
